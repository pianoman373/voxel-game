...
#define LOOKUP_TABLE_SIZE 12288
...
void ChunkIO::saveChunk(Chunk *c) {
    if (!c->changedFromDisk) {
        //std::cout << "skipping chunk that didn't change" << std::endl;
        return;
    }
    ...
    mx.lock();
    std::string filename = "save/" + std::to_string(c->chunk_x >> 4) + "_" + std::to_string(c->chunk_y >> 4) + "_" + std::to_string(c->chunk_z >> 4) + ".voxeldata";
    ...
    int chunkIndex = ((c->chunk_x & 15) + (c->chunk_z & 15) * 16 + (c->chunk_y & 15) * 256) * 3;
    ...
    uint8_t compressedChunk[32*32*32*2];
...
void ChunkIO::loadChunk(Chunk *c) {
    mx.lock();
    if (!c) {
        mx.unlock();
        return;
...
bool ChunkIO::chunkHasFile(Chunk *c) {
    std::string filename = "save/" + std::to_string(c->chunk_x >> 5) + "_" + std::to_string(c->chunk_y >> 5) + "_" + std::to_string(c->chunk_z >> 5) + ".voxeldata";

    return fileExists(filename);
}

bool ChunkIO::chunkIsSaved(Chunk *c) {
    std::string filename = "save/" + std::to_string(c->chunk_x >> 5) + "_" + std::to_string(c->chunk_y >> 5) + "_" + std::to_string(c->chunk_z >> 5) + ".voxeldata";
